...
public class NamaskaraGenerator extends Observable{
	int sYear;
	int eYear;
	int sMonth;
	int eMonth;
	...
	public NamaskaraGenerator(int sYear, int eYear, int sMonth, int eMonth,
	...
	public String baseGenerator() {
		String url = "http://www.namaskara.lk/?sid=article&dt=" + year + "/"
...
//   		 	HttpURLConnection uc = (HttpURLConnection) url.openConnection(proxy);
		HttpURLConnection uc = (HttpURLConnection) url.openConnection();
			...
			try {
				...
				Elements urlList = doc.select("ul").get(0).select("ul").get(0).select("li");
